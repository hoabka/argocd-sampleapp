    ...
    stage('docker-push-registry') {
     ...
     stage('Deploy DEV') {
      steps {
        sh '''#!/usr/bin/env bash
          ...
          cd ./argocd-k8s-manifest/dev && kustomize edit set image ${REGISTRY}/samplewebapp=${REGISTRY}/samplewebapp:${GIT_COMMIT}
     ...
     stage('Deploy PROD') {
      steps {
        ...
        sh '''#!/usr/bin/env bash
          cd ./argocd-k8s-manifest/prod && kustomize edit set image ${REGISTRY}/samplewebapp=${REGISTRY}/samplewebapp:${GIT_COMMIT}
